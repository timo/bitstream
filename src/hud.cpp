...
void DrawSpeedBar(){
  ...
  GLdouble speedfactor, speed;
  speedfactor = playerptr->GetVelocity()/100.0;
  // energyfactor = 1.0;
  speed = HUD_LEFT_FG + HUD_ENERGY_DIFF*speedfactor;
  ...
  glVertex3f(HUD_LEFT_BG, HUD_SPEED_TOP_BG, -1);
  glVertex3f(HUD_RIGHT_BG, HUD_SPEED_TOP_BG, -1);
  glVertex3f(HUD_LEFT_BG, HUD_SPEED_BOTTOM_BG, -1);

  glVertex3f(HUD_RIGHT_BG, HUD_SPEED_BOTTOM_BG, -1);
  glVertex3f(HUD_RIGHT_BG, HUD_SPEED_TOP_BG, -1);
  glVertex3f(HUD_LEFT_BG, HUD_SPEED_BOTTOM_BG, -1);

  glColor4f(0.2, 0.8, 0.2, 1.0);

  glVertex3f(HUD_LEFT_FG, HUD_SPEED_TOP_FG, -0.999);
  glColor4f(0.8, 0.8, 0.8, 1.0);
  glVertex3f(speed, HUD_SPEED_TOP_FG, -0.999);
  glColor4f(0.2, 0.8, 0.2, 1.0);
  glVertex3f(HUD_LEFT_FG,HUD_SPEED_BOTTOM_FG , -0.999);
  glColor4f(0.8, 0.8, 0.8, 1.0);
  glVertex3f(speed, HUD_SPEED_BOTTOM_FG, -0.999);
  glVertex3f(speed, HUD_SPEED_TOP_FG, -0.999);
  glColor4f(0.2, 0.8, 0.2, 1.0);
  glVertex3f(HUD_LEFT_FG, HUD_SPEED_BOTTOM_FG, -0.999);
...
void DrawEnergyBar(){
  ...
  // energyfactor = 1.0;
  energy = HUD_LEFT_FG + HUD_SPEED_DIFF*energyfactor;
